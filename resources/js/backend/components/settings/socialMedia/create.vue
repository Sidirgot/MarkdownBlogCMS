<template>
    <modal name="social-create" height="auto" :pivotY="0.2" @closed="closed" :adaptive="true">
         <div class="p-4 bg-main-dark text-white">
            <h1 class="text-lg border-b border-navbar pb-2">Add A New Social Media Account</h1>
            ...
            <div class="flex my-4 -mx-4">
                <button class="flex-1 btn btn-blue mx-4" @click="$modal.hide('social-create')">Cancel</button>
                <button class="flex-1 btn btn-indigo mx-4" @click="createAccount">Create</button>
            </div>
        </div>
    </modal>
</template>
...
export default {
    ...
    methods: {
        createAccount() {
            axios.post('/oath/socialMedia', this.account)
            .then( res => {
                Bus.$emit('flash-message',{text: 'Social Media Account Added Successfully', type: 'success'})
                this.$emit('refresh')
                this.$modal.hide('social-create')
            })
            .catch( e => {
                Bus.$emit('flash-message', {bag: e.response.data.errors,type: 'error',});
            })
        },
        closed(event) {
            this.account = {}
